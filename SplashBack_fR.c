...
double Gama,Di,dvir,Ri,a,rc,R0;	//R0 scaling units, Ri more physical edge
...
//  return a *pow(3.0 *Mi /(4.0 *pi *Om *rhocr *(1.0 + d[ind_r])), 1.0 /3);
  if(ind_r>ind_rvir)
    return R0 *ri[ind_r] *a /ai *pow((1.0 +di[ind_r])/(1.0 +d[ind_r]) ,1.0 /3);
  else
    return R0 *ri[ind_r] *avir[ind_r] /ai *pow((1.0 +di[ind_r])/(1.0 +d[ind_r]) ,1.0 /3);
}
...
const double epsilon2=1.5*(H0c*H0c*Om)*pow(1.0+lm_ratio,-2);	

//	fR correction to the source term
double epsilon(double k){
  double k0=k/get_rad(ind_r0);	//	physical units
  double k2=k0*k0*fR0;
  return k2/(k2 *3.0 + a *a *epsilon2 *pow(pow(a,-3) +lm_ratio,3) );
}
...
void calc_source(void){
  ...
  if(ind_rvir>=ind_ri){	//	orviting from the halo's edge
    source[ind_ri]=0.0;
    for(ind_k=0;ind_k<n_k-1;ind_k++)
      source[ind_ri]+=(k[ind_k+1]-k[ind_k])
...
double r2prime(double r1, double r2, double a, double source){
  ...
  -r1/abs(r1)  *pow(r1,-2) *3.0 *Om /(2.0 *E2a2(a)) *source *pow(a /avir[ind_ri], 3.0);}
...
void set_initial_profile(int type){
  ...
  R0 = ai *pow(3.0 *Mi /(4.0 *pi *Om *rhocr *(1.0 + Di*0.5    )), 1.0 /3);
  Ri = ai *pow(3.0 *Mi /(4.0 *pi *Om *rhocr *(1.0 + di[ind_ri])), 1.0 /3);
...
void set_NFW(void){
  Rv=get_rad(ind_ri);	//	half of the turn around radius for the selected shell tat represents the edge of the halo
  for(int id_r=0;id_r<n_r;id_r++)
    d1[id_r]=NFW(r[id_r],3.0);		//	3.0 is the concentration
  ind_rvir=n_r;	//	stopping the collapsing process of every shell, everyone will remain frozen at the NFW value
}
...
//printing epsilon initial
//for(double kk=0.01;kk<3.;kk+=0.01)
//printf("%le %le\n",kk,epsilon(kk));
  ...
  for(int id_r=0;id_r<n_r;id_r++){
    ...
    if((ind_rvir<n_r)&&(rvir[id_r]<0.0)&&(rta[id_r]>0.0)&&( rta[id_r]*0.5 >get_rad(id_r) )){// 3=conc, may we vary it with z?
      ...
      if(id_r==ind_ri){	//	if edge just virilized then initialize orbit
        ...
        avir[ind_r0]=a;		//	needed for scaling k
