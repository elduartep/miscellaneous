...
const int   NumMax_bin_rad_halos=200;		//	520 resultaron ser muchos, se demora demasiado
const int   NumMax_bin_rad_voids=NumMax_bin_rad_halos;
const int   Num_bin_stack_halos=10;
...
//const double Num_bines_masa[5]={250.,500.,1000.,5000.,10000.};                               // 10¹¹ Ms/h
//const double Num_bines_radio[5]={1.*nc/Lbox,2.*nc/Lbox,5.*nc/Lbox,10.*nc/Lbox,20.*nc/Lbox};  // unidades de nc
//double       Num_bines[5];


//	Num_bines radiales perfil para los diferentes bines radiales stack
//16 const int Num_bin_rad_voids[Num_bin_stack_halos]={200,200,200,200,150,150,150,150,100,100,100,100,100,100,100,100};
...
const    double rad_max_voids=10.;		//	radio maximo para los Num_bines lineales, unidades rv
					//	equivale a 200 Num_bines
...
printf("alloca_perfiles_individuales...\n");fflush(stdout);
  ...
  for(int p = 0; p < NumObjetos; p++) 
    if(!(perfil_individual[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory perfil_individual.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( perfil_individual2= (double **) malloc(NumObjetos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < NumObjetos; p++) 
    if(!(perfil_individual2[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory perfil_individual2.\n");
      fflush(stderr);
      exit(0);}
...
printf("alloca perfiles+errores octantes...\n");fflush(stdout);
  ...
  for(int p = 0; p < JN; p++) 
    for(int q = 0; q < Num_bin_stack; q++) 
      if(!(perfil_octante[p][q] = (double *) malloc( NumMax_bin_rad_halos * sizeof(double)))){
        fprintf(stderr, "failed to allocate memory perfil_octante.\n");
        fflush(stderr);
        exit(0);}
  ...
  for(int p = 0; p < JN; p++) 
    for(int q = 0; q < Num_bin_stack; q++) 
      if(!(perfil_octante2[p][q] = (double *) malloc( NumMax_bin_rad_halos * sizeof(double)))){
        fprintf(stderr, "failed to allocate memory perfil_octante2.\n");
        fflush(stderr);
        exit(0);}
  ...
  for(int p = 0; p < JN; p++) 
    for(int q = 0; q < Num_bin_stack; q++) 
      if(!(error_perfil_octante[p][q] = (double *) malloc( NumMax_bin_rad_halos * sizeof(double)))){
        fprintf(stderr, "failed to allocate memory error_perfil_octante.\n");
        fflush(stderr);
        exit(0);}
  ...
  for(int p = 0; p < JN; p++) 
    for(int q = 0; q < Num_bin_stack; q++) 
      if(!(error_perfil_octante2[p][q] = (double *) malloc( NumMax_bin_rad_halos * sizeof(double)))){
        fprintf(stderr, "failed to allocate memory error_perfil_octante2.\n");
        fflush(stderr);
        exit(0);}
...
printf("alloca_perfiles_medios...\n");fflush(stdout);
  ...
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(perfil_medio[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory perfil_medio.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( perfil_medio2= (double **) malloc(Num_bin_stack_halos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(perfil_medio2[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory perfil_medio2.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( error_perfil_medio= (double **) malloc(Num_bin_stack_halos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(error_perfil_medio[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory error_perfil_medio.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( error_perfil_medio2= (double **) malloc(Num_bin_stack_halos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(error_perfil_medio2[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory error_perfil_medio2.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( perfil_medioI= (double **) malloc(Num_bin_stack_halos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(perfil_medioI[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory perfil_medio.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( perfil_medioI2= (double **) malloc(Num_bin_stack_halos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(perfil_medioI2[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory perfil_medio2.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( error_perfil_medioI= (double **) malloc(Num_bin_stack_halos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(error_perfil_medioI[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory error_perfil_medio.\n");
      fflush(stderr);
      exit(0);}
  ...
  if(!( error_perfil_medioI2= (double **) malloc(Num_bin_stack_halos * sizeof(double *)))){
    ...
    exit(0);}
  for(int p = 0; p < Num_bin_stack_halos ; p++) 
    if(!(error_perfil_medioI2[p] = (double *) malloc(NumMax_bin_rad_halos * sizeof(double)))){
      fprintf(stderr, "failed to allocate memory error_perfil_medio2.\n");
      fflush(stderr);
      exit(0);}
...
printf("calcula pefil_densidad...\n");fflush(stdout);
  ...
  for(l=0;l<NumObjetos;l++)
    for(b=0;b<NumMax_bin_rad_halos;b++){
...
//printf("l=%d carga=%le, ",l,Mo[l]);fflush(stdout);
    ...
    if((Mo[l]>=carga_min)&&(Mo[l]<carga_max)){
      ...
      max_stack*=5./Num_bin_rad_voids[Num_bin_stack_halos-1];   //	da 10,7.5,5 o 5 dependiendo el Num_bin_stack
...
for(l=0;l<NumObjetos;l++){
  ...
  for(l=0;l<NumObjetos;l++){
    ...
    if((Mo[l]>=carga_min)&&(Mo[l]<carga_max)){
      ...
      max_stack*=5./Num_bin_rad_voids[Num_bin_stack_halos-1];
...
//    Num_bin=NumMax_bin_rad_voids;
    Num_bin=Num_bin_rad_voids[0];
    Num_bin_stack=Num_bin_stack_voids;

//    for(int p=0;p<=Num_bin_stack;p++)
  //    Num_bines[p]=Num_bines_radio[p];
