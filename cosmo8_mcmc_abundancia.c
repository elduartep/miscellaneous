// volviendo a usar dos potencias porque el ajuste no es visualmente bueno con solo una
...
void carga(void){
  //	alpha: potencia
  param[0]=5.45e-00;	sigma[0]=3.e-01;	//	4
  param[1]=5.30e-00;	sigma[1]=3.e-01;	//	5
  param[2]=5.15e-00;	sigma[2]=3.e-01;	//	6
  param[3]=5.5e-00;	sigma[3]=3.e-01;	//
  param[4]=5.8e-00;	sigma[4]=3.e-01;	//	A
  param[5]=6.0e-00;	sigma[5]=3.e-01;	//	B
  param[6]=6.0e-00;	sigma[6]=3.e-01;	//	D

  param[7]=9.0e-01;	sigma[7]=5.e-02;	//	A
  param[8]=4.1e-00;	sigma[8]=1.e-01;	//	beta/ segunda potencia
  param[9]=1.5e-00;	sigma[9]=5.e-02;	//	c: factor dentro de la exponencial
...
return param[7]*pow(sigma,-alpha)*(1.+pow(sigma,param[8]))*exp(-param[9]*pow(sigma,-2));
}
...
//	media
  for(i=first_theory;i<=3;i++)
    fprintf(SA,"%le %le %le %le %le %le %le %le %le\n",parametro[i]
     ,mean[i],sqrt(stdev[i]),mean[7],sqrt(stdev[7]),mean[8],sqrt(stdev[8]),mean[9],sqrt(stdev[9]));
//	best fit
  for(i=first_theory;i<=3;i++)
    fprintf(SA,"%le %le %le %le %le %le %le %le %le\n",parametro[i]
     ,pMin[i],sqrt(stdev[i]),pMin[7],sqrt(stdev[7]),pMin[8],sqrt(stdev[8]),pMin[9],sqrt(stdev[9]));
  fclose(SA);
...
//	media
  for(i=3;i<=last_theory;i++)
    fprintf(SA,"%le %le %le %le %le %le %le %le %le\n",parametro[i]
     ,mean[i],sqrt(stdev[i]),mean[7],sqrt(stdev[7]),mean[8],sqrt(stdev[8]),mean[8],sqrt(stdev[8]));
//	best fit
  for(i=3;i<=last_theory;i++)
    fprintf(SA,"%le %le %le %le %le %le %le %le %le\n",parametro[i]
     ,pMin[i],sqrt(stdev[i]),pMin[7],sqrt(stdev[7]),pMin[8],sqrt(stdev[8]),pMin[8],sqrt(stdev[8]));
  fclose(SA);
